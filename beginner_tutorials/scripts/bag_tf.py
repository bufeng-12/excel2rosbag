...
with open('/home/zengjun/workset_zongmu_slam/radar2bag/odom2_1_leijia.txt') as dataset:
    with rosbag.Bag('/home/zengjun/workset_zongmu_slam/Gmapping/radar_simu_tf2_1_leijia.bag', 'w') as bag:
        for line in dataset.readlines():
            line = line.strip()
            tokens = line.split(' ')
            num_scans = 720
            if len(tokens) <= 2:
                ...
                tf_msg = make_tf_msg(odom_x, odom_y, odom_theta, t, 'odom_combined', 'base_link')
                bag.write('tf', tf_msg, t)
